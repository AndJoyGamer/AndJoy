...
class MonsterLogic extends YASpriteDomainLogic<MonsterDomain>
{
	private float fFrames;
	private MainActivity activity;
	private Vec2 vecAntiGrav;

	protected MonsterLogic(World world, MainActivity activity)
	{
...
//		fInitX_M = -60;
	...
	{
		MonsterState.WAIT.setStateClocker(new WaitClocker());
		MonsterState.WALK.setStateClocker(new WalkClocker());
		MonsterState.JUMP.setStateClocker(new JumpClocker());
		MonsterState.ATTACK1.setStateClocker(new Attack1Cloker());

		// 待机到行走
		builder.newTransition().from(MonsterState.WAIT)
				.to(MonsterState.WALK)
				.on(domainContext.TO_WALK);
		// 行走到待机
		builder.newTransition().from(MonsterState.WALK)
				.to(MonsterState.WAIT)
		...
		builder.newTransition().from(MonsterState.WAIT)
				.to(MonsterState.JUMP)
				.on(domainContext.TO_JUMP).perform(action);
		builder.newTransition().from(MonsterState.WALK)
				.to(MonsterState.JUMP)
				.on(domainContext.TO_JUMP).perform(action);
		builder.onEntry(MonsterState.JUMP).perform(
				new JumpEnterAction());

		// 待机到攻1
		// 行走到攻1
		builder.newTransition().from(MonsterState.WAIT)
				.to(MonsterState.ATTACK1)
				.on(domainContext.TO_ATTACK1);
		builder.newTransition().from(MonsterState.WALK)
				.to(MonsterState.ATTACK1)
				.on(domainContext.TO_ATTACK1);
		builder.onEntry(MonsterState.ATTACK1).perform(
	...
	private void resetState()
	{
		if (activity.bLeftPressing || activity.bRightPressing)
			stateMachine.forceSetState(MonsterState.WALK);
		else
			stateMachine.forceSetState(MonsterState.WAIT);
	}
	...
	private class FootContactLsn implements YIOnContactListener
	{
				...
				YABaseDomain domainOther)
		{
			...
			++iFootContact;
			if (MonsterState.JUMP == stateMachine.getCurrentState())
				resetState();
		}
				...
				YABaseDomain domainOther)
		{
			...
			--iFootContact;
			if (iFootContact == 0)
				if (MonsterState.JUMP != stateMachine
						.getCurrentState())
					stateMachine.forceSetState(MonsterState.JUMP);
		}
	}
}
